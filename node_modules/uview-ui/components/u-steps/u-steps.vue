		...
		<view
			...
			<view class="u-steps__item" 
				...
				<view
					...
					v-if="mode == 'number'"
					:style="{borderColor: current < index ? unActiveColor : activeColor}">
					<text :style="{color: current < index ? unActiveColor : activeColor,}" >
						{{ index + 1 }}
					</text>
				...
				<text class="u-line-1" :style="{color: index <= current ? '#333' : unActiveColor,}" :class="['u-steps__item__text--' + direction]">
					<view style="text-align: left;font-size: 34rpx;" v-if="item.title">{{item.title}}</view>
					<view >
						{{ item.name }}
					</view>
					<view v-if="index == 0 && item.photo" class="imgs">
						<image v-for="(it,ind) in item.photo" :key="ind" @click="kanimg(item.photo,ind)" :src="it" mode="widthFix"></image>
					</view>
					<view v-if="index == 3 && item.complainantPhoto" class="imgs">
						<image v-for="(it,ind) in item.complainantPhoto" :key="ind" @click="kanimg(item.complainantPhoto,ind)" :src="it" mode="widthFix"></image>
					</view>
					<view v-if="index == 3 && item.otherPhoto" class="imgs">
						<image v-for="(it,ind) in item.otherPhoto" :key="ind" @click="kanimg(item.otherPhoto,ind)" :src="it" mode="widthFix"></image>
					</view>
				</text>
				<view class="u-steps__item__line" :class="['u-steps__item__line--' + mode]" v-if="index < list.length - 1">
					<u-line :direction="direction" length="90%" :hair-line="false" :color="unActiveColor"></u-line>
				</view>
			</view>
		</view>
	</view>
...
x				
		...
		.u-line-1{
			...
			.imgs{
				...
				width: 100%;
				image{
					width: 160rpx;height: 160rpx;border-radius: 10rpx;margin: 4rpx;
				}
			}
		}
		&--column {
